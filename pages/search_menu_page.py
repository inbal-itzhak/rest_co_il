...
class SearchMenuPage(BasePage):
    ...
    def search_restaurant_explicit(self, search_string):
        ...
        time.sleep(2)
        # search_box.click()
        allure.attach(body=f"{search_string}", name="search string", attachment_type=allure.attachment_type.TEXT)
    ...
    def search_partial_text(self, search_string):
        try:
            ...
            if autocomplete_options:
                allure.attach(body=f"{len(autocomplete_options)}", name="number of options in autocomplete",
                              attachment_type=allure.attachment_type.TEXT)
            ...
            allure.attach(body=f"{traceback.format_exc()}", name="exception thrown",
                          attachment_type=allure.attachment_type.TEXT)
            # print(f"{e.with_traceback()}")

    @allure.step("search restaurant with partial texts and without selecting from autocomplete, search string:"
                 "  {search_string}")
    ...
    def search_partial(self, search_string):
        ...
        if search_box:
            ...
            print(f"sent text {search_string}")
            autocomplete_options = (WebDriverWait(self.driver, 10).
                                    until(ec.presence_of_all_elements_located(self.AUTOCOMPLETE_OPTIONS)))
            allure.attach(body=f"{len(autocomplete_options)}", name="number of options in autocomplete",
                          attachment_type=allure.attachment_type.TEXT)
            # print(f"autocomplete has {len(autocomplete_options)} values")
            if len(autocomplete_options) > 0:
                for option in autocomplete_options:
                    try:
                        ...
                        allure.attach(body=f"{option_text}", name="text of autocomplete option",
                                      attachment_type=allure.attachment_type.TEXT)
                        # print(f"text of autocomplete{option_text}")
                        if search_string in option_text:
                            self.click_el(option)
                            allure.attach(body=f"{search_string} is equal {option_text}",
                                          name="search string is equal to autocomplete option",
                                          attachment_type=allure.attachment_type.TEXT)
                            # print(f"{search_string} is equal {option_text}")
                            return
                        else:
                            allure.attach(body=f"{search_string} is not equal {option_text}",
                                          name="search string is not equal to autocomplete option",
                                          attachment_type=allure.attachment_type.TEXT)
